...
- ライブラリ内の動画検索（キーワード＝タイトル/タグ/保存コメント、タグ AND・投稿者・解像度・再生時間・ショートでの絞り込み）
- 検索結果の並び替え（DL 日時/投稿日/タイトル/再生数/マイリスト数/コメント数/再生時間/再生回数/最終再生/ランダム）とページング
- ライブラリの集計表示（本数・総再生時間・投稿者数・タグ数と、よく付くタグ／解像度の分布。クリックでそのまま絞り込み）
- 保存済みコメントの横断検索（ヒットしたコメントからその再生位置へ頭出し）
...
- Phase 1.3: ライブラリ層の拡張（検索/整列/集計など）（実装済み。ライブラリ画面への UI 接続も完了）
